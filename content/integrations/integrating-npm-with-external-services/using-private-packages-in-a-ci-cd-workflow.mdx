...
If trusted publishing is not available for your CI/CD provider, you may create an [automation token][create-token] or a [granular access token][create-token] configured to bypass 2FA on the website. This will allow you to publish even if you have two-factor authentication enabled on your account.
